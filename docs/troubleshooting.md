...
- Windows task has no visible window: inspect the daily `scheduled-YYYY-MM-DD.log` under the user cache directory and query it with `schtasks.exe /Query /TN "AKEF SKPort Daily Claim" /V /FO LIST`.
